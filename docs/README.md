# SynapseWorks documentation

Public documentation source for the SynapseWorks GitBook project.

## Products

- [RME: PDF & Document Scanner](rme-pdf-scanner/README.md) — an open-source, privacy-first Android document scanner.

## Site resources

- [Support](https://synapseworks.org/support/)
- [Security](https://synapseworks.org/security/)
- [SynapseWorks privacy policy](https://synapseworks.org/privacy/)
- [Link checks](link-checks.md)
